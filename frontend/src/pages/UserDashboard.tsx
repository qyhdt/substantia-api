...
const ENDPOINTS: Record<Fmt, { titleKey: TKey; noteKey: TKey; curl: (k: string) => string }> = {
  anthropic: {
...
// Cursor 经 OpenAI 兼容接入：Base URL 必须带 /v1；自定义模型名用 claude4.8
// （避免和 Cursor 自带的 "Opus 4.8" 撞名走订阅；后台 normalize_model 会归一到 claude-opus-4-8）
const CURSOR_BASE_URL = 'https://api.substantia.ai/v1'
const CURSOR_MODEL = 'claude4.8'
              ...
              {expanded && (
                <div className="ak-accordion-b">
                  ...
                  <div className="ak-row" style={{ justifyContent: 'flex-end', gap: 8, marginBottom: 6 }}>
                    <CopyBtn text={e.curl('<你的 sk-key>')} label={t('copy_sample')} />
      ...
      <Card title={t('card_cursor')}>
        ...
        <ol style={{ margin: '8px 0 4px', paddingLeft: 20, lineHeight: 1.9 }}>
          ...
          <li>
            ...
            <div className="ak-row" style={{ gap: 8, margin: '6px 0', alignItems: 'center' }}>
              <code className="ak-mono">{CURSOR_MODEL}</code>
              <CopyBtn text={CURSOR_MODEL} />
            </div>
          </li>
          <li>{t('cursor_step6_1')}<span className="ak-mono">{CURSOR_MODEL}</span>{t('cursor_step6_2')}<b>{t('cursor_step6_3')}</b>{t('cursor_step6_4')}</li>
        </ol>
        {banner && (
        ...
        <p className="ak-muted" style={{ fontSize: 12, marginTop: 8 }}>
          {t('cursor_foot_1')}<span className="ak-mono">{CURSOR_MODEL}</span>{t('cursor_foot_2')}<span className="ak-mono">claude-opus-4-8</span>{t('cursor_foot_3')}<span className="ak-mono">@文件 / @文件夹</span>{t('cursor_foot_4')}
        </p>
      </Card>
