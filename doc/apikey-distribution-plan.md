...
## 11. 变更记录（changelog）
- **2026-07-22** · 用户与管理后台统计图统一升级为按需加载的 Apache ECharts 6：每日消费使用平滑渐变面积折线，费用分布使用圆角交互式环形图，支持动画、悬浮明细和 `ResizeObserver` 响应式缩放。
- **2026-07-22** · 用户“我的账单”增加每日消费曲线，随 7/30/90 天与 RMB/USD 选择同步刷新，并与管理后台复用同一折线图组件。
- **2026-07-22** · 修复费用环形图图例越界：长模型/渠道名称在可用列内省略，百分比和价格保持右对齐且不再溢出卡片。
- **2026-07-22** · 全站账单、费用图表、调用明细、余额和模型价格的展示金额统一保留两位小数；底层 micro-USD/micro-CNY 账本与供应商精确对账明细不改变精度。
- **2026-07-22** · 用户“我的账单”在概览和调用明细之间增加模型费用、渠道费用两张环形分布图；账单接口新增按渠道聚合，并与管理后台复用同一图表组件、配色、占比和 RMB/USD 换算规则。
- **2026-07-22** · 管理用量看板对标墨行账单概览：支持北京时间自定义区间、近 7 天/本周/本月/上月快捷筛选，增加总消费、上期环比、日均、峰值、请求数统计；消费趋势叠加上期虚线，并增加模型费用和渠道费用环形分布图。
