...
## 11. 变更记录（changelog）
- **2026-07-22** · 管理后台用量币种展示优化：「按模型」看板、用户详情按模型汇总及最近调用中，GLM、Kimi 等国内模型按当前动态 USD/CNY 汇率显示人民币，海外模型继续显示美元；余额与按用户/slot 等混合汇总维持美元口径。
...
- **2026-07-22** · 用户「调用明细」列表统一换算人民币：无论 Claude、GPT、GLM 或 Kimi，模型币种标记均显示 CNY，花费按当前动态 USD/CNY 汇率显示为 `¥`；仅影响展示，不改历史 micro-USD 账务记录。
