...
- [x] 支持按 USD/RMB 录入充值和墨行余额快照，人民币记录保留当时汇率；可核对内部账差和墨行余额差
- [x] 商务后台可分别调整官网价、墨行给我们的成本折扣和我们对客户的销售折扣；销售价自动同步到客户计费表
- [x] 日维度、模型维度和请求级明细展示销售额、供应商成本、毛利及实付贡献；成本未定价/usage 缺失单独告警

**对账定义**：`充值 + 调账 - 墨行请求成本 = 供应商账面余额`；`账面余额 - 墨行余额快照 = 对账差异`；
`客户销售额 - 供应商成本 = 毛利`；`客户实付扣款 - 供应商成本 = 现金贡献`。供应商余额允许为负数，
...
## 11. 变更记录（changelog）
- **2026-07-22** · **墨行供应商成本账本与请求级对账**。管理后台新增「墨行对账」：支持录入 USD/RMB 充值、调账和供应商余额快照；分别维护官网价、上游成本折扣和客户销售折扣。每次墨行请求在客户扣费同一事务中写入供应商成本流水并扣减供应商账面余额，保存上游实际模型和所有商务快照；提供余额守恒、墨行余额差、日/模型/请求级销售额、成本、毛利、实付贡献和异常请求核查。普通用户接口仅公开销售定价，不暴露供应商折扣与成本。历史请求不回填虚构成本，从 migration `0020` 部署时点开始精确追踪。
